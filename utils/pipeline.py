...
class StableHairPipeline(DiffusionPipeline):
    ...
    def __init__(
        ...
        # ─── 4. CHANNELS_LAST FOR CONVS ─────────────────────────────────────────
        # This tells PyTorch to lay out conv feature maps in NHWC (channels_last),
        # which MKL-DNN will run much faster on CPU.
        # self.unet   = self.unet.to(memory_format=torch.channels_last)
        # self.controlnet = self.controlnet.to(memory_format=torch.channels_last)

        # ─── 5. DYNAMIC QUANT FOR linears ──────────────────────────────────────
        # Wrap only the nn.Linear layers in int8 dynamic quantization.
        from torch.quantization import quantize_dynamic
        self.unet         = quantize_dynamic(self.unet,         {torch.nn.Linear}, dtype=torch.qint8)
        self.text_encoder = quantize_dynamic(self.text_encoder, {torch.nn.Linear}, dtype=torch.qint8)
    ...
    def prepare_condition(self, condition, device, dtype, do_classifier_free_guidance):
        if isinstance(condition, torch.Tensor):
            ...
            condition = self.images2latents(condition, dtype)
        if do_classifier_free_guidance:
    ...
    def __call__(
        ...
        print("____________________________test: cls", type(ref_image))
        ...
        if isinstance(ref_image, str):
            ...
            ref_image_array = np.array(ref_image.resize((width, height)))
            ref_image_latents = self.images2latents(ref_image_array, latents_dtype)
